...
def cartesian_control(joint_transforms, b_T_ee_current, b_T_ee_desired,
    ...
    transform_difference = tf.transformations.concatenate_matrices(ee_T_b, b_T_ee_desired)

    angle, axis = rotation_from_matrix(transform_difference)

    position_delta = transform_difference[0:3, 3].T

    rot_delta = numpy.dot(angle, axis)

    if numpy.linalg.norm(position_delta) > 0.1:
        position_delta /= numpy.linalg.norm(position_delta)

    if numpy.linalg.norm(rot_delta) > 1.0:
        rot_delta /= numpy.linalg.norm(rot_delta)

    position_gain = 1.0
    rot_gain = 1.0

    Delta_X = numpy.concatenate((position_delta * position_gain, rot_delta * rot_gain), axis=0)
    ...
    for jt in joint_transforms:
        ee_T_joints.append(tf.transformations.concatenate_matrices(ee_T_b, jt))

    velocity_transform_matrices = []

    for jt in ee_T_joints:
        rev = tf.transformations.inverse_matrix(jt)
        j_trans_ee = tf.transformations.translation_from_matrix(rev)
        ss = S_matrix(j_trans_ee)
        first_row = numpy.append(jt[:3, :3], (numpy.dot((-jt[:3, :3]), ss)), axis=1)
        second_row = numpy.append(numpy.zeros([3, 3]), jt[:3, :3], axis=1)
        V_j = numpy.append(first_row, second_row, axis=0)
        velocity_transform_matrices.append(V_j)

    revolute_joint_velocity_transforms = []

    for v in velocity_transform_matrices:
        revolute_joint_velocity_transforms.append(v[:, 5])

    J = numpy.empty((6, num_joints))

    J = numpy.column_stack(revolute_joint_velocity_transforms)

    # ------------------------ compute the q_dot using the pseudo-inverse of J ---------------- #
    J_plus = numpy.linalg.pinv(J)
    dq = numpy.dot(J_plus, Delta_X)
    # ----------------------------------------------------------------------

    if red_control:

        delta_q = numpy.array([q0_desired - q_current[0], 0, 0, 0, 0, 0, 0])
        q_n = numpy.dot((numpy.identity(7) - numpy.dot(J_plus, J)), delta_q)
